...
# forecastable: the realized factor return on day t+1 is FAC_PRED-correlated with a score
# that was knowable on day t, plus an independent shock. That shock is the risk a vendor
# cannot design away, and it is what keeps the naive Sharpe realistic instead of infinite.
...
style_load = rng.normal(0.0, 1.0, (N_ASSETS, N_STYLE))   # each stock's loading on each factor
...
shock = rng.standard_normal((N_DAYS, N_STYLE))           # independent day-t+1 factor shocks
...
returns = market[:, None] * beta[None, :] + style + idio   # shape (N_DAYS, N_ASSETS)

# --- 1.4 cross-sectional standardisation (rank the 400 names against each other, per day) ---
...
# factor scores perfectly would report on day t. It is rank N_STYLE (low breadth) AND it is
# only a partial predictor of the realized return, because the shock is not in it. This is
# the single quantity a real vendor is competing to estimate.
alpha = (FAC_PRED * score[:-1]) @ style_load.T           # (N_DAYS-1, N_ASSETS)
alpha = (alpha - alpha.mean(axis=1, keepdims=True)) / alpha.std(axis=1, keepdims=True)
...
def style_noise():
    """Fat-tailed error that lives in the SAME style subspace as the signal, plus a little
    pure idiosyncratic noise. Standardised per day to unit cross-sectional variance.

    Living in the style subspace is what makes Step 2 honest: every vendor's book bets the
    same handful of style factors (fixed breadth), so a vendor that is mostly error is
    making mostly WRONG style bets and earns a genuinely low Sharpe, while a strong vendor's
    signal dominates and earns a high one. That is what spreads the real vendors by IC and
    lets the weak ones overlap the lucky fakes.

    The style-space part is PERSISTENT (AR(1)), like real vendor data: a vendor that is
    accidentally long a trending factor stays long it, so the Step 2 smoothing configs can
    lock onto that lucky exposure and turn pure noise into an impressive in-sample Sharpe.
    """
    in_style = ar1_fat(N_DAYS - 1, N_STYLE, NOISE_PHI) @ style_load.T   # rank N_STYLE, alpha's space
...
def make_vendor(target_ic):
    ...
    Signal and error both live in the style subspace (see style_noise), so the naive book
    always bets the same low number of style factors. s is chosen so the realised full-sample
    cross-sectional IC lands on target_ic: since alpha correlates with the true return at
    rho_pz, a signal that is a fraction s of alpha has IC ~ s * rho_pz.
    """
    noise = style_noise()
    if target_ic == 0:
        return noise                                         # pure null vendor
    s = target_ic / rho_pz                                   # fraction of signal to inject
...
# --- Step 1 acceptance criteria: print a sanity block, all must hold ---
...
print(f"  mean pairwise corr     {sample_corr:.3f}   (want ~0.25-0.35, market factor present)")
...
print(f"  factor subspace K      {N_STYLE}   (rho_pz = {rho_pz:.3f}, the IC ceiling)")
